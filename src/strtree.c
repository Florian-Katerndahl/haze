...
int buildGEOSGeometriesFromGDAL(const char *filePath, const char *layerName, vectorGeometryList **geometries, const char *inputReferenceSystem) {
    int retVal = 0;
    assert(fileReadable(filePath));

    GDALAllRegister();

    GDALDatasetH *vectorDataset = GDALOpenEx(filePath, GDAL_OF_VECTOR | GDAL_OF_READONLY, NULL, NULL, NULL);
    ...
    if (needsReprojection) {
        ...
        transformation = OCTNewCoordinateTransformationEx(sourceReferenceSystem, targetReferenceSystem, NULL);
    ...
    OGR_FOR_EACH_FEATURE_BEGIN(feature, layer) {
        ...
        GEOSGeometry *geosMBR = GEOSGeom_createRectangle(mBr->MinX, mBr->MinY, mBr->MaxX, mBr->MaxY); // todo how is error handling done?
        ...
        vecGeom->geometry = GEOSWKBReader_read(reader, OGRWkb, OGR_G_WkbSize(geom)); // todo how is error handling done?
        vecGeom->mbr = geosMBR;
...
GEOSSTRtree *buildSTRTreefromRaster(const struct averagedData *data, const struct geoTransform *transformation, cellGeometryList **cells) {
    ...
    for (size_t x = 0; x < data->columns; x++) {
        for (size_t y = 0; y < data->rows; y++) {
            double x1 = transformation->xOrigin + (double) x * transformation->pixelWidth + (double) y * transformation->rowRotation;
            double x2 = transformation->xOrigin + ((double) x + 1.0) * transformation->pixelWidth + (double) y * transformation->rowRotation;
            double y1 = transformation->yOrigin + (double) x * transformation->colRotation + (double) y * transformation->pixelHeight;
            double y2 = transformation->yOrigin + (double) x * transformation->colRotation + ((double) y + 1.0) * transformation->pixelHeight;
            ...
            cell->geometry = geom;
            cell->value = *data->data + x + y * data->columns; // todo make clear, that ownership is not passed and dataset handle is stillresponsible for freeing data
...
intersection_t *querySTRTree(vectorGeometryList *areasOfInterest, GEOSSTRtree *rasterTree) {
    ...
    while (areasOfInterest != NULL) {
        ...
        GEOSSTRtree_query(rasterTree, areasOfInterest->entry->mbr, queryCallback, (void *) &node->intersectingCells);
